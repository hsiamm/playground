...
class plgButtonModulesAnywherePopup
{
	function render(&$params)
	{
		...
		if ($search) {
			$where[] = 'LOWER( m.title ) LIKE '.$db->q('%'.$db->getEscaped($search, true).'%', false);
		}
		if ($filter_state != '') {
		...
		$query->leftJoin('#__users AS u ON u.id = m.checked_out');
		$query->leftJoin('#__viewlevels AS g ON g.id = m.access');
		...
		$query->from('#__modules as m');
		$query->where('m.client_id = '.(int) $client->id);
		...
		$query->where('e.client_id = '.(int) $client->id);
		...
		foreach ($types as $i=> $type) {
			...
			$path = $client->id ? JPATH_ADMINISTRATOR : JPATH_SITE;
		...
		$script = "
			function modulesanywhere_jInsertEditorText( id, modulepos ) {
				...
				if ( modulepos ) {
					...
					str = '{".$tag." '+id;
		";
		if ($params->override_style && (count(explode(',', $params->styles)) > 1 || $params->styles != $params->style)) {
			$script .= "
