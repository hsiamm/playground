						...
						for($i=1; $i<255; $i++) {
								...
								if (!empty($search_to) || !empty($search_from)) {
									...
									foreach($items as $item) {
										...
										preg_match("/\"{$i}\",\"value\":\"([0-9]{4}-[0-9]{2}-[0-9]{2}|\d+|\d.+)\"/", $extra, $match);
										...
										else if (empty($search_from) && !empty($search_to)) {
											if($value <= $search_to) {
												$myids[] = $item->id;
											}
										}
                               ...
                               if (! empty($search)) {
								...
								if(!empty($title)) {
									...
									foreach($titles as $k => $title) {
										$query .= " AND (i.title REGEXP '^.*{$title}.*$' OR i.introtext REGEXP '^.*{$title}.*$' OR i.fulltext REGEXP '^.*{$title}.*$')";
									}
								}
								...
								if(!empty($ftext)) {
									$query .= " AND (i.introtext REGEXP '^.*{$ftext}.*$' OR i.fulltext REGEXP '^.*{$ftext}.*$')";
								}								
								...
								///item all in one
								$fitem_all = JRequest::getString('fitem_all');
								...
								if(!empty($fitem_all)) {
									...
									if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish'.DS.'joomfish.php')) {
											...
											$sql .= " WHERE jfc.value = ".$db->Quote($fitem_all);
											$sql .= " AND jfc.reference_table = 'k2_tags'";
											$sql .= " AND jfc.reference_field = 'name' AND jfc.published=1";
									...
									if (!isset($result) || $result < 1) {
											$sql = "SELECT id FROM #__k2_tags WHERE name=".$db->Quote($fitem_all);
											$db->setQuery($sql, 0, 1);
											$result = $db->loadResult();
									}
									
									$query .= " AND ((i.id IN (SELECT itemID FROM #__k2_tags_xref WHERE tagID=".(int)$result."))";
									$query .= " OR (i.title REGEXP '^.*{$fitem_all}.*$')";
									$query .= " OR (i.introtext REGEXP '^.*{$fitem_all}.*$')";
									$query .= " OR (i.fulltext REGEXP '^.*{$fitem_all}.*$')";
									//$query .= " OR (i.extra_fields_search REGEXP '^.*{$fitem_all}.*$')";
									$query .= ")";
								...
								if(!empty($created_from) || !empty($created_to)) {
									if(!empty($created_from) && !empty($created_to)) {
										$query .= " AND i.created >= '{$created_from} 00:00:00' AND i.created <= '{$created_to} 23:59:59'";
									}
									if(!empty($created_from) && empty($created_to)) {
										$query .= " AND i.created >= '{$created_from} 00:00:00'";
									}
									if(empty($created_from) && !empty($created_to)) {
										$query .= " AND i.created <= '{$created_to} 23:59:59'";
									}
								}								
								...
								if(!empty($price_from) || !empty($price_to)) {
									...
									foreach($items as $item) {
										...
										$price = $params->get("k2storeitem_price");
                                ...
                                if($slider == 0 and $array == 0) {
                                        ...
                                        if ($type == 'exact') {
                                                ...
                                                if ($n != 1) {
													...
													$search_upper = $json->encode(mb_strtoupper($search));
													$search_upper = str_replace("\"", "", $search_upper);
													$search_upper = str_replace("\\", "\\\\\\\\", $search_upper);
													
													$search_lower = $json->encode(mb_strtolower($search));
													$search_lower = str_replace("\"", "", $search_lower);
													$search_lower = str_replace("\\", "\\\\\\\\", $search_lower);
													
													$search_upper_first = $json->encode(mb_substr($search_upper, 0, 1) . mb_strtolower(mb_substr($search, 1)));
													...
													if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_joomfish'.DS.'joomfish.php') && 	$currentLang != $defaultLang) {
														...
														$sql = " AND (";
														$sql .= "(i.extra_fields REGEXP '^.*\"$i_slider\",\"value\":\"[^}]*{$search}.*\".*$')";
