...
///added changed all 'Item' to ItemFilter
class K2ModelItemFilter extends JModel
{
	...
	function prepareItem($item, $view, $task){
		...
		//added
		|| ($view=='itemlist' && $task=='filter')
		//added
		
		)
		{
			$tags = K2ModelItemFilter::getItemTags($item->id);
		...
		)
		{
			...
			if($template != "ja_wall") {
				$item->extra_fields=K2ModelItemFilter::getItemExtraFields($item->extra_fields);
			}
			//added
		}
		...
		{
			$item->attachments=K2ModelItemFilter::getItemAttachments($item->id);
		}
		...
		{
			$item->votingPercentage = K2ModelItemFilter::getVotesPercentage($item->id);
			$item->numOfvotes = K2ModelItemFilter::getVotesNum($item->id);
		...
		if ($item->params->get('catItemIntroTextWordLimit') && ($task=='category' || $task=='filter')) {
		...
		)
		{
			if (!empty($item->created_by_alias)){
				...
				$item->author->profile = K2ModelItemFilter::getUserProfile($item->created_by);
		...
		if(!$user->guest && $user->id==$item->created_by && $params->get('inlineCommentsModeration')){
			$item->numOfComments = K2ModelItemFilter::countItemComments($item->id, false);
		}
		else {
			$item->numOfComments = K2ModelItemFilter::countItemComments($item->id);
		}
		return $item;
	}
	...
	function prepareFeedItem(&$item){
		...
		if($params->get('feedItemAttachments')){
			$attachments = K2ModelItemFilter::getItemAttachments($item->id);
	...
	function execPlugins($item, $view, $task){
		...
		//added
		|| ($view=='itemlist' && $task == 'filter' && $item->params->get('userItemK2Plugins'))
	...
	function getVotesNum($itemID=NULL) {
		...
		$vote = K2ModelItemFilter::getRating($itemID);
