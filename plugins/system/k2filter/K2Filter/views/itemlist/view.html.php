	...
	function display($tpl = null) {
		...
		switch ($task) {
			...
			case 'filter':
				...
				$module = &JModuleHelper::getModule('k2_filter');
				$moduleParams = new JParameter($module->params);
				
				$filter_template = JRequest::getVar("template_id");
				if($filter_template == '') {
					$filter_template = $moduleParams->get('results_template', 0);
				}
				...
				switch ($filter_template) {
					...
					case 0 :
						...
						$resultf = JRequest::getVar("resultf");
						
						if($model->getTotal() == 0) {
							$resultf = JRequest::getVar("noresult"); 
						}
												
						//get extras for ordering
						if(JRequest::getVar("ordering_extra") != '') {
							$exclude = $moduleParams->get('ordering_extra_exclude', '');	
							$extras = $model->getExtra($exclude);
						}
						
						$this->assignRef('resultf', $resultf);
						$this->assignRef('result_count', $model->getTotal());
					...
					case 2 :
						...
						// Get category
						$category_id = JRequest::getInt('restcat');
						if($category_id == 0) {
							$category_id = JRequest::getInt('restcata');
						}
						if($category_id == 0) {
					...
					if(JRequest::getVar("page_heading") != "") {
						$title .= JRequest::getVar("page_heading");
					}
