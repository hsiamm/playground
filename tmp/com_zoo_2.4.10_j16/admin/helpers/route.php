...
class RouteHelper extends AppHelper {
	...
	protected $_item_links, $_category_links = array();
	...
	public function alphaindex($application_id, $alpha_char = null) {
		...
		if (($menu_item = $this->_findFrontpage($application_id)) || ($menu_item = JSite::getMenu()->getActive())) {
	...
	public function category($category) {
		...
		if (($menu_item = $this->_findCategory($category->id)) || ($menu_item = $this->_findInCategoryPath($category))) {
			...
			if (($menu_item = $this->_findFrontpage($category->application_id)) || ($menu_item = JSite::getMenu()->getActive())) {
				$link .= '&Itemid='.$menu_item->id;
			}
		}
