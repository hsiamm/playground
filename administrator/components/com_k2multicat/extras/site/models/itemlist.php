<?php
/**
 * @version		$Id: itemlist.php 1281 2011-10-27 17:06:26Z lefteris.kavadas $
 * @package		K2
...
class K2ModelItemlist extends JModel {
		...
		function getData($ordering = NULL) {
				...
				//$query.=" FROM #__k2_items as i LEFT JOIN #__k2_categories AS c ON c.id = i.catid";
				  $query.=" FROM #__k2_items as i LEFT JOIN #__k2_categories AS c ON FIND_IN_SET( c.id , i.catid )";
...
//										$query .= " AND c.id IN ({$sql})";
		...
		function getTotal() {
				...
				$query = "SELECT COUNT(*) FROM #__k2_items as i LEFT JOIN #__k2_categories c ON FIND_IN_SET( c.id , i.catid )";
...
//				$query.=" FROM #__k2_items as i LEFT JOIN #__k2_categories AS c ON FIND_IN_SET( c.id , i.catid )";
 ...
 function countCategoryItems($id) {
        ...
        $categories = K2ModelItemlist::getCategoryTree($id, true);
        ...
        foreach ($catIDs as $catid) {
              ...
              $query = "SELECT COUNT(*) FROM #__k2_items WHERE FIND_IN_SET( $catid ,  catid )  AND published=1 AND trash=0";
