...
<div class="catItemView group<?php echo ucfirst($this->item->itemGroup); ?><?php echo ($this->item->featured) ? ' catItemIsFeatured' : ''; ?><?php if($this->item->params->get('pageclass_sfx')) echo ' '.$this->item->params->get('pageclass_sfx'); ?>">
  ...
  <div class="catItemLinks">
		...
		<div class="catItemCategory">
			...
			<?php
				$db = JFactory::getDBO();
				$sql = "SELECT id,name FROM #__k2_categories WHERE id IN(".$this->item->catid.")";
				$db->setQuery($sql);
				$cats = $db->loadObjectList();  
				
				for($x=0; $x<count($cats);$x++){
				  ?>
				  <a href="<?php echo JRoute::_('index.php?option=com_k2&view=itemlist&task=category&id='.$cats[$x]->id);?>"><?php echo $cats[$x]->name; ?></a><?php if($x < count($cats)-1) echo ", ";?>
				  <?php
				}
				?>
