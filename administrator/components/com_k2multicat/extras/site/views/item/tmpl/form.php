...
<script type="text/javascript">
	...
	function submitbutton(pressbutton) {
		...
		//added by prakash
		var myele = document.getElementById('catid');
		var catid = false;
		for(i=0; i<myele.options.length;i++){
				if(myele.options[i].selected && myele.options[i].value!='0' ){
					catid = true;
				}
			}
	...
	window.addEvent('domready', function(){
		...
		$('catid').addEvent('change', function(){
			...
			var cid = '';
			for(i=0; i<myele.options.length;i++){
				if(myele.options[i].selected){
					cid += myele.options[i].value + ',' ;
				}
			}
			var url = 'index.php?option=com_k2&view=item&task=extraFields&cid='+cid;
			//end
			<?php if ($this->row->id): ?>
			      ...
			      <td valign="top">
							...
							<table class="adminform adminFormK2">
								...
								<tr>
								 ...
								 <tr>
		                        <td valign="top"><label><?php echo JText::_('SEF / Breadcrumb category'); ?></label></td>
									<td><?php echo $this->lists['sefcatid']; ?></td>
		                        </tr>
							</table>
