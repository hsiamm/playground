<?php
/**
 * @version		$Id: view.html.php 1267 2011-10-27 15:52:11Z lefteris.kavadas $
 * @package		K2
...
class K2ViewItem extends JView
{
	...
	function display($tpl = null) {
		...
		$query = "SELECT ordering AS value, title AS text FROM #__k2_items WHERE catid in ( {$item->catid} )";
		...
		$lists['sefcatid'] = JHTML::_('select.genericlist', $categories, 'sefcatid', 'class="inputbox"', 'value', 'text', @$item->sefcatid);
...
//		$category->load($item->catid);
		...
		foreach($cats as $category){
			if(@in_array($category->extraFieldsGroup,$tmparray)) continue;
				$tmparray[] = $category->extraFieldsGroup;
			if($item->id)
		...
		foreach($extraFields as $ke=>$extraField){
		...
		for($i=0; $i< @sizeof(@$newarray); $i++){
			$newarray[$i]->element=$extraFieldModel->renderExtraField($newarray[$i],$item->id);
		}


		$extraFields = @$newarray;
