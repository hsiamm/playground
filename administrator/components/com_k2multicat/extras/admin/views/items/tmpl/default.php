	...
	<table class="adminlist">
		...
		<tbody>
			...
			<tr class="row<?php echo ($key%2); ?>">
				...
				<?php
				$db = JFactory::getDBO();
				$sql = "SELECT id,name FROM #__k2_categories WHERE id IN(".$row->catid.")";
				$db->setQuery($sql);
				$cats = $db->loadObjectList();  
				
				for($x=0; $x<count($cats);$x++){
				  ?>
				  <a href="<?php echo JRoute::_('index.php?option=com_k2&view=category&cid='.$cats[$x]->id);?>"><?php echo $cats[$x]->name; ?></a><?php if($x < count($cats)-1) echo ", ";?>
				  <?php
				}
				?>
