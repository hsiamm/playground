<?php
/**
 * @version		$Id: item.php 1267 2011-10-27 15:52:11Z lefteris.kavadas $
 * @package		K2
...
class K2ModelItem extends JModel {
	...
	function save($front = false) {
		...
		//added by prakash
		$catids =  JRequest::getVar('catid', null, 'post', 'array');
		if($catids[0]==0)	 unset($catids[0]); 
		
		$row->catid =  @implode(',',$catids); 
		//end
		
		if ($front && $row->id == NULL) {
		
			for($x=0;$x<count($catids);$x++){
				if($catids[$x] <=0) continue;
					JLoader::register('K2HelperPermissions', JPATH_SITE.DS.'components'.DS.'com_k2'.DS.'helpers'.DS.'permissions.php');
				if (!K2HelperPermissions::canAddItem($catids[$x])) {
