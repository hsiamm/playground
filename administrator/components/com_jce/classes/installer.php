<?php
/**
 * @version   $Id: installer.php 224 2011-06-12 16:29:11Z happy_noodle_boy $
 * @package   JCE
	...
	function checkTable($table)
	{
		...
		if (!empty($tables)) {
			...
			foreach($tables as $item) {
				if (preg_match('#^([A-Za-z0-9]+)_' . str_replace('#__', '', $table) . '$#i', $item)) {
					return true;
				}
			}
    ...
    function checkEditorVersion()
    {
        ...
        if (!JFile::exists($file)) {
            ...
            if ($xml = JApplicationHelper::parseXMLInstallFile($file)) {
                ...
                // Development version
                if (strpos($this->version, '2.0.0RC2') !== false || strpos($version, '2.0.0RC2') !== false) {
                    return true;
                }
