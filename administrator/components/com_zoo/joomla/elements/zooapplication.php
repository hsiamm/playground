...
class JElementZooApplication extends JElement {
	...
	function fetchElement($name, $value, &$node, $control_name) {
		...
		// init vars
		$params = $this->_parent instanceof JParameter ? $this->_parent : $app->data->create($this->_parent->getValues());
		...
		foreach ($table->all(array('order' => 'name')) as $application) {
			...
			if ($node->attributes('categories')) {
				...
				$opts    = $node->attributes('frontpage') ? array($app->html->_('select.option', '', '&#8226;	'.JText::_('Frontpage'))) : array();
		...
		$javascript  = 'jQuery("#'.$name.'").ZooApplication({ url: "'.$link.'", msgSelectItem: "'.JText::_('Select Item').'" });';
