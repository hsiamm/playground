					...
					if (is_subclass_of($element, 'ElementRepeatable')) {
						...
						foreach ($element as $index => $instance) {
							if (is_a($element, 'ElementDate')) {
								...
								$data[$identifier][$index] = $instance->getElementData()->getParams();
							}
						}
					} else {
						$data[$identifier] = $element->getElementData()->getParams();
					}
				}
            }
        }
    ...
    public function bind($value) {
        ...
        try {
			...
			if ($element = $this->_form->getItem()->getElement($this->_name)) {
				...
				// get AppData value
				$value = $value ? $this->app->data->create($value) : $this->app->data->create();
